...
for (year, product_id) in set((key[0], key[1]) for key in yearproductidword_2_count.keys()):
    word_2_yearproductid = {k[2]: v for k, v in yearproductidword_2_count.items() if k[0]==year and k[1]==product_id} # parole che hanno year = year e id = id
    sorted_word_by_year = {k: v for k,v in sorted(word_2_yearproductid.items(), key=lambda item: item[1], reverse=True)} # parole ordinate
    for k, v in list(sorted_word_by_year.items())[:5]: #top 5 parole per quel product id
